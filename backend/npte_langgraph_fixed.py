...
print(f"🔧 Setup: OpenAI={OPENAI_AVAILABLE}")
...
class NPTEState(TypedDict):
    ...
    web_context: Optional[str]  # Choice-specific web abstracts
...
async def generate_question_node(state: NPTEState, config: RunnableConfig) -> NPTEState:
    ...
    try:
        ...
        if pdf_result.get("status") == "success" and pdf_result.get("pdf_chunks"):
            ...
            print("⚠️ No PDF chunks found, proceeding with web search only")
        
        # STEP 2: Get web abstracts using the user-selected topic (1st call to journals for question generation)
        web_context = ""
        try:
            from web_sources import get_fast_web_snippet
            print(f"🔍 1st call to journals: Topic-focused search for question generation")
            # Use the topic for initial question generation (choices don't exist yet)
            web_snippet = get_fast_web_snippet(topic)
            if web_snippet and web_snippet.get("snippet_text"):
                # Use full content if available, otherwise use abstract
                web_context = web_snippet.get("full_content") or web_snippet["snippet_text"]
                content_type = web_snippet.get("content_type", "abstract")
                print(f"✅ Got topic-focused web {content_type} for question generation")
            else:
                print("⚠️ No topic-focused abstracts found for question generation")
        except Exception as e:
            print(f"⚠️ Topic-based web snippet failed: {e}")
        
        # STEP 3: Load few-shot examples
        few_shot_examples = load_few_shot_examples()
        
        # STEP 4: Create enhanced system prompt with both PDF and web context
        context_section = ""
        if pdf_context:
            context_section += f"\n\nPDF KNOWLEDGE CONTEXT:\n{pdf_context[:1500]}"  # Limit context length
        if web_context:
            context_section += f"\n\nWEB ABSTRACTS (topic-based):\n{web_context[:900]}"
...
"""
        ...
        # Store web context for later use in evaluation
...
async def evaluate_answer_node(state: NPTEState, config: RunnableConfig) -> NPTEState:
    ...
    try:
        ...
        # Get choice-specific abstracts for detailed explanations
        user_choice_text = ""
        ...
        print(f"📚 Getting choice-specific abstracts for detailed explanations")
        
        # Extract keywords from all choices for comprehensive context
        all_choices = choices
        stop_words = {"the", "a", "an", "and", "or", "but", "in", "on", "at", "to", "for", "of", "with", "by", 
                     "should", "would", "could", "will", "most", "best", "appropriate", "likely", "patient", "following"}
        
        # Collect all choice phrases for comprehensive context
        all_choice_phrases = []
        for choice in all_choices:
            clean_choice = choice.replace('A. ', '').replace('B. ', '').replace('C. ', '').replace('D. ', '')
            all_choice_phrases.append(clean_choice)
        
        print(f"🔍 Using all choice phrases for explanations: {[phrase[:30] + '...' if len(phrase) > 30 else phrase for phrase in all_choice_phrases]}")
        
        # Get choice-specific abstracts for detailed explanations (2nd call to journals)
        explanation_articles = []
        try:
            from web_sources import get_fast_web_snippet
            
            # Use the full phrases from all 4 choice answers - no keyword filtering
            # This preserves the complete context and specificity of each choice
            search_query = ' '.join(all_choice_phrases)
            
            print(f"🔍 2nd call to journals: Choice-specific search for explanations")
            print(f"🔍 Using targeted explanation query: '{search_query}'")
            web_snippet = get_fast_web_snippet(search_query)
            if web_snippet and web_snippet.get("snippet_text"):
                # Use full content if available, otherwise use abstract
                content = web_snippet.get("full_content") or web_snippet.get("snippet_text", "")
                content_type = web_snippet.get("content_type", "abstract")
                
                explanation_articles.append({
                    "title": web_snippet.get("title", ""),
                    "journal": web_snippet.get("journal", ""),
                    "abstract": web_snippet.get("snippet_text", ""),
                    "full_content": content,
                    "content_type": content_type,
                    "url": web_snippet.get("url", ""),
                    "source": web_snippet.get("source_domain", "")
                })
                print(f"✅ Got choice-specific {content_type} for explanations")
            else:
                print(f"⚠️ No choice-specific abstracts found for explanations")
        except Exception as e:
            print(f"⚠️ Choice-specific article search failed: {e}")
        
        # Also use cached PDF knowledge
        ...
        if cached_pdf.get("status") == "success" and cached_pdf.get("pdf_chunks"):
            ...
            if contextual_keywords:
                for chunk in cached_pdf["pdf_chunks"]:
                    ...
                    keyword_matches = sum(1 for keyword in contextual_keywords if keyword in content)
                    if keyword_matches >= max(1, len(contextual_keywords) // 2):
            ...
            if relevant_chunks:
                ...
                print(f"⚠️ No relevant PDF context found for choice keywords")
        ...
        # Include PDF context and full article content in explanation
        context_section = ""
        if pdf_explanation_context:
            context_section = f"\n\nPDF KNOWLEDGE FOR EXPLANATION:\n{pdf_explanation_context[:1000]}"
        
        # Add choice-specific content for detailed explanations
        if explanation_articles:
            for article in explanation_articles:
                content_type = article.get('content_type', 'abstract')
                content = article.get('full_content', article.get('abstract', ''))
                
                context_section += f"\n\nCHOICE-SPECIFIC JOURNAL {content_type.upper()} FOR EXPLANATIONS:\n"
                context_section += f"Title: {article['title']}\n"
                context_section += f"Journal: {article['journal']}\n"
                context_section += f"Source: {article['source']}\n"
                context_section += f"Content: {content[:2000]}\n"
                print(f"📖 Including choice-specific {content_type}: {article['title'][:50]}...")
        
        # Add original web context if available (topic-based) - FALLBACK ONLY
        web_context = state.get("web_context", "")
        if web_context and not explanation_articles:
            context_section += f"\n\nWEB ABSTRACTS (topic-based):\n{web_context[:600]}"
            print("⚠️ FALLBACK: Using topic-based abstracts instead of choice-specific abstracts for explanations")
        elif web_context and explanation_articles:
            print("✅ Using choice-specific abstracts for explanations (no fallback needed)")
        else:
            print("⚠️ No web context available for explanations")
...
"""
        ...
        # Get learning materials using fast API sources (replacing Tavily)
        user_choice_text = ""
        correct_choice_text = ""
        ...
        # Get study links for ALL 4 answer choices (one link per choice subject)
        study_links = []
        try:
            from web_sources import get_fast_web_snippet
            
            print(f"🔍 Getting study materials for all 4 answer choices...")
            
            # Extract keywords from each choice individually
            stop_words = {"the", "a", "an", "and", "or", "but", "in", "on", "at", "to", "for", "of", "with", "by", 
                         "should", "would", "could", "will", "most", "best", "appropriate", "likely", "patient", "following"}
            ...
            for i, choice in enumerate(choices):
                if choice:
                    ...
                    # Use the entire phrase of the answer - no keyword filtering or truncation
                    # This preserves the full context and specificity of each choice
                    search_query = clean_choice
                    
                    print(f"📚 Searching for choice {chr(65+i)}: {clean_choice[:30]}...")
                    print(f"🔍 Using targeted query: '{search_query}'")
                    
                    web_snippet = get_fast_web_snippet(search_query)
                    if web_snippet and web_snippet.get("url"):
                        choice_links[chr(65+i)] = web_snippet["url"]
                        study_links.append(web_snippet["url"])
                        print(f"✅ Found link for choice {chr(65+i)}: {web_snippet.get('title', 'No title')[:40]}...")
                    else:
                        print(f"❌ No link found for choice {chr(65+i)}")
            
            # Fallback strategy: If we don't have all 4 links, prioritize correct + user's choice
            if len(study_links) < 4:
                print(f"⚠️ Only found {len(study_links)}/4 links, using fallback strategy...")
                
                # Ensure we have materials for the correct choice
                if correct_answer is not None and correct_answer < len(choices):
                    correct_choice = choices[correct_answer].replace('A. ', '').replace('B. ', '').replace('C. ', '').replace('D. ', '')
                    if chr(65+correct_answer) not in choice_links:
                        # Use only the choice content, no topic mixing
                        search_query = correct_choice
                        web_snippet = get_fast_web_snippet(search_query)
                        if web_snippet and web_snippet.get("url"):
                            study_links.append(web_snippet["url"])
                            print(f"🎯 Added fallback link for correct choice")
                
                # Ensure we have materials for user's choice (if different from correct)
                if user_answer is not None and user_answer != correct_answer and user_answer < len(choices):
                    user_choice = choices[user_answer].replace('A. ', '').replace('B. ', '').replace('C. ', '').replace('D. ', '')
                    if chr(65+user_answer) not in choice_links:
                        # Use only the choice content, no topic mixing or hard-coded terms
                        search_query = user_choice
                        web_snippet = get_fast_web_snippet(search_query)
                        if web_snippet and web_snippet.get("url"):
                            study_links.append(web_snippet["url"])
                            print(f"📚 Added fallback link for user's choice")
            
            print(f"📖 Total study links found: {len(study_links)}/4")
            
            # If we have PDF context but fewer than 2 links, provide journal info from PDFs
            if len(study_links) < 2 and pdf_explanation_context:
                print("📚 Adding journal information from PDF embeddings...")
                # Extract journal names and titles from PDF chunks
                pdf_journal_info = []
                if cached_pdf.get("pdf_chunks"):
                    for chunk in cached_pdf["pdf_chunks"][:3]:  # Get top 3 PDF sources
                        source = chunk.get("source", "Unknown")
                        metadata = chunk.get("metadata", {})
                        title = metadata.get("title", "Unknown title")
                        journal = metadata.get("journal", "Unknown journal")
                        
                        if journal != "Unknown journal" and title != "Unknown title":
                            # Mark PDF references as non-clickable research suggestions
                            pdf_journal_info.append(f"📖 Please research this paper: {journal}: {title}")
                            print(f"📖 Added PDF journal info: {journal}: {title[:50]}...")
                
                # Add PDF journal info as study materials
                if pdf_journal_info:
                    study_links.extend(pdf_journal_info)
                    print(f"📚 Added {len(pdf_journal_info)} PDF journal references")
            
            # If fewer than 4 links (one per choice), use web crawler as final fallback
            if len(study_links) < 4:
                print(f"🕷️ Only found {len(study_links)}/4 links, using web crawler for remaining choices...")
                try:
                    from web_sources import get_full_article_content
                    
                    # Try to get full article content for choices that don't have links yet
                    missing_choices = []
                    for i, choice in enumerate(choices):
                        if choice and chr(65+i) not in choice_links:  # A, B, C, D not found yet
                            missing_choices.append((i, choice))
                    
                    print(f"🕷️ Missing links for choices: {[chr(65+i) for i, _ in missing_choices]}")
                    
                    # Try to get full article content from explanation articles first
                    if explanation_articles:
                        for article in explanation_articles:
                            if article.get("url") and len(study_links) < 4:
                                full_content = get_full_article_content(article["url"])
                                if full_content:
                                    study_links.append(f"📖 Full Article: {article['title']} - {article['url']}")
                                    print(f"🕷️ Added full article content: {article['title'][:50]}...")
                                    break  # Only add one full article to avoid too much content
                    
                    # If still missing links, try to search for specific missing choices
                    if len(study_links) < 4 and missing_choices:
                        print("🕷️ Searching for specific missing choice content...")
                        for i, choice in missing_choices[:2]:  # Limit to 2 additional searches
                            if len(study_links) >= 4:
                                break
                            clean_choice = choice.replace('A. ', '').replace('B. ', '').replace('C. ', '').replace('D. ', '')
                            search_query = f"{topic} {clean_choice}"
                            print(f"🕷️ Crawler search for choice {chr(65+i)}: {clean_choice[:30]}...")
                            
                            # Try to get content for this specific choice
                            web_snippet = get_fast_web_snippet(search_query)
                            if web_snippet and web_snippet.get("url"):
                                full_content = get_full_article_content(web_snippet["url"])
                                if full_content:
                                    study_links.append(f"📖 Full Article: {web_snippet['title']} - {web_snippet['url']}")
                                    print(f"🕷️ Added full article for choice {chr(65+i)}: {web_snippet['title'][:40]}...")
                                    
                except Exception as e:
                    print(f"⚠️ Web crawler failed: {e}")
                
                print(f"🕷️ Final study links count: {len(study_links)}/4")
                    
        except Exception as e:
            print(f"⚠️ Fast API search failed: {e}")
            # Fallback to generic Physiopedia/APTA links
            study_links = [
                f"https://physiopedia.com/search?q={topic.replace(' ', '+')}",
                f"https://www.apta.org/search?q={topic.replace(' ', '+')}"
            ]
            print(f"📚 Using fallback Physiopedia/APTA links")
        
        print(f"📋 Total study links: {len(study_links)}")
...
class NPTEProfessorAgent:
    ...
    async def generate_mcq(self, topic: str, session_id: str = "default") -> Dict:
        ...
        try:
            ...
            # Cache for evaluation (including PDF knowledge and web context for reuse)
            cached_data = {
    ...
    async def evaluate_answer(self, session_id: str, user_answer_index: int) -> Dict:
        ...
        try:
            ...
            # Create evaluation state with cached data (including PDF knowledge and web context)
            eval_state = {
                ...
                "web_context": cached_data.get("web_context", ""),
